...
async function fetchCsrf() {
    try {
        ...
        if (success) document.getElementById('csrf_token').value = token;
...
async function saveAppointment(method, body, successMsg) {
    ...
    } catch (err) { showError(err.message || 'API failure. Please check your connection.'); }
...
async function deleteAppointment(id, name) {
    ...
    try {
        const data = await sendJson('DELETE', { id, csrf_token: field('csrf_token') });
        if (data.success) {
    ...
    } catch (err) { showToast(err.message || 'API failure. Could not delete.', 'error'); }
}

async function updateStatus(id, status, selectEl) {
    try {
        const data = await sendJson('PUT', { id, status, csrf_token: field('csrf_token') }, '?action=status');
        if (data.success) {
    ...
    } catch (err) { showToast(err.message || 'API failure.', 'error'); await loadAppointments(); }
}
// ── Slot Check ──
...
// ── Form State ──
function cancelEdit() { 
    resetForm(); document.querySelectorAll('.appt-row').forEach(r => r.classList.remove('editing')); }

function resetForm() {
    const token = field('csrf_token');
...
function statusEmoji(s) { 
    return { Pending: '⏳', Confirmed: '✅', Cancelled: '❌' }[s] || ''; }
